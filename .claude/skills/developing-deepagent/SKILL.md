...
allowed-tools:
  ...
  - AskUserQuestion
---
# Developing Ai Comment's DeepAgent

Implement DeepAgent components using the `deepagents` SDK within Ai Comment Act patterns.
...
```bash
uv add --package comment-writer deepagents

# For the QuickJS code interpreter (Step §"Interpreter"):
uv add --package comment-writer "deepagents[quickjs]"
```
